  ...
  const renderActivitySection = (step) => {
    ...
    return (
      <div className="space-y-6">
        ...
        {data.interactiveElements && data.interactiveElements.includes('wound-selector') && (
          <div className="bg-gradient-to-r from-blue-100 to-purple-100 rounded-lg p-6 border border-blue-200">
            ...
            <p className="text-gray-700 mb-4">Select the wounds that resonate with your experience:</p>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {['Rejection', 'Abandonment', 'Neglect', 'Criticism/Shame', 'Betrayal', 'Humiliation'].map(wound => (
                <label key={wound} className="flex items-center space-x-2 p-3 bg-white rounded-lg border border-gray-200 hover:border-purple-300 cursor-pointer">
                  <input type="checkbox" className="w-4 h-4 text-purple-600" />
                  <span className="text-sm text-gray-700">{wound}</span>
                </label>
              ))}
            </div>
          </div>
        ...
        {data.interactiveElements && data.interactiveElements.includes('belief-mapper') && (
          <div className="bg-gradient-to-r from-pink-100 to-orange-100 rounded-lg p-6 border border-pink-200">
            ...
            <textarea 
              ...
              placeholder="Examples: 'I'm unlovable,' 'I'm not good enough,' 'I must be perfect...'"
              rows={4}
            />
          </div>
        )}

        {data.interactiveElements && data.interactiveElements.includes('manager-identifier') && (
          <div className="bg-gradient-to-r from-blue-100 to-teal-100 rounded-lg p-6 border border-blue-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {['The Perfectionist', 'The People-Pleaser', 'The Planner', 'The Critic', 'The Caretaker', 'The Controller'].map(manager => (
        ...
        {data.interactiveElements && data.interactiveElements.includes('six-fs-wizard') && (
          <div className="bg-gradient-to-r from-purple-100 to-indigo-100 rounded-lg p-6 border border-purple-200">
            ...
            <div className="space-y-4">
              {['Find', 'Focus', 'Flesh Out', 'Feel Toward', 'Befriend', 'Fear'].map((step, index) => (
                <div key={step} className="flex items-start space-x-3 p-4 bg-white rounded-lg border border-gray-200">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600 mt-1">
                      {step === 'Find' && 'Notice when a part is active in your system'}
                      {step === 'Focus' && 'Direct your compassionate attention to the part'}
                      {step === 'Flesh Out' && 'Explore the part\'s role and perspective'}
                      {step === 'Feel Toward' && 'Notice your emotional response to the part'}
                      {step === 'Befriend' && 'Build trust and understanding with the part'}
                      {step === 'Fear' && 'Ask what the part fears would happen if it stopped'}
                    </p>
                  </div>
                </div>
              ))}
        ...
        {data.interactiveElements && data.interactiveElements.includes('readiness-assessment') && (
          <div className="bg-gradient-to-r from-green-100 to-blue-100 rounded-lg p-6 border border-green-200">
            ...
            <div className="space-y-4">
              <div className="p-4 bg-white rounded-lg border border-gray-200">
                <p className="text-gray-700 mb-3">Can you reliably access Self-energy when parts are active?</p>
                <div className="flex space-x-4">
                  <label className="flex items-center space-x-2">
                    <input type="radio" name="self-energy" className="w-4 h-4" />
                    <span className="text-sm">Yes, consistently</span>
                  </label>
                  <label className="flex items-center space-x-2">
                    <input type="radio" name="self-energy" className="w-4 h-4" />
              ...
              <div className="p-4 bg-white rounded-lg border border-gray-200">
                <p className="text-gray-700 mb-3">Do you have support available if intense emotions arise?</p>
                <div className="flex space-x-4">
                  <label className="flex items-center space-x-2">
                    <input type="radio" name="support" className="w-4 h-4" />
                    <span className="text-sm">Yes, therapist/friends</span>
                  </label>
                  <label className="flex items-center space-x-2">
                    <input type="radio" name="support" className="w-4 h-4" />
                    <span className="text-sm">Some support</span>
                  </label>
                  <label className="flex items-center space-x-2">
                    <input type="radio" name="support" className="w-4 h-4" />
