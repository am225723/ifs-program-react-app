...
- **Guided Onboarding Flow**: Multi-step welcome wizard for new clients on first login. 4 steps: Welcome, What is IFS, How the App Helps, Get Started (with CTA to take wound assessment). Completion tracked per-client in localStorage. Skippable. Only shown to clients (not advisors). Component: `src/components/OnboardingFlow.jsx`.
